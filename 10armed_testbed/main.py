...
if __name__ == "__main__":
    ...
    for eps_idx, eps in enumerate(eps_list):
        print("Running simulation for eps=0.0")
        ...
        plt.plot(avg_reward_list[eps_idx], color=colors[eps_idx])
    plt.xlabel("Steps")
    plt.ylabel("Average reward")
    plt.xlim(0, 1000)
    plt.ylim(0, 1.5)
    plt.savefig("./outputs/fig22.jpg")
    # plt.show()

    plt.figure(figsize=(16, 8))
    for eps_idx in range(len(eps_list)):
        plt.plot(percent_opt_action_list[eps_idx], color=colors[eps_idx])
    plt.xlabel("Steps")
    plt.ylabel("% Optimal Action")
    plt.xlim(0, 1000)
    plt.ylim(0, 1)
    plt.savefig("./outputs/fig23.jpg")
    # plt.show()